...
def _build_csv(hazards: List[HazardData], generated_by: str) -> str:
    ...
    buffer = io.StringIO()
    buffer.write(f"# AGAILA Export - Generated by {generated_by} - EPSG:4326\n")
...
async def export_geojson(
    ...
    try:
        ...
        response = FileResponse(
            ...
            filename=f"agaila_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.geojson",
        )
        response.headers["X-AGAILA-Spatial-Reference"] = EXPORT_SPATIAL_REFERENCE
        response.headers["X-AGAILA-Data-License"] = EXPORT_DATA_LICENSE
        response.headers["X-AGAILA-Generated-By"] = generated_by
        ...
        await log_system_error(request, e, context="geojson_export")
        raise HTTPException(
...
async def export_csv(
    ...
    try:
        ...
        response = FileResponse(
            path=str(output_path),
            media_type="text/csv",
            filename=f"agaila_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
        )
        response.headers["X-AGAILA-Generated-By"] = generated_by
        background_tasks.add_task(lambda: output_path.unlink(missing_ok=True))
        return response
