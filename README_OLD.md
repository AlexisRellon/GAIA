...
GAIA is deployed on Railway using Docker containers. For complete deployment instructions, see [`docs/guides/RAILWAY_DEPLOYMENT.md`](docs/guides/RAILWAY_DEPLOYMENT.md).
...
# Deploy all services (backend, frontend, redis, celery-worker, celery-beat)
railway up
...
- `CORS_ORIGINS` - Allowed CORS origins (e.g., `https://*.up.railway.app`)

See [Railway Deployment Guide](docs/guides/RAILWAY_DEPLOYMENT.md) for full configuration details.
```
